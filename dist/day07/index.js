...
class Step {
  constructor(value, dependencies, parents) {
    this.done = false;
    this.value = value;
  ...
  isDoable() {
    return (this.parents.length === 0 || this.parents.every((parent) => parent.done)) && !this.done;
  }
  startProcessing() {
    this.isPorcessing = true;
  }
  isTempDoable() {
    return (this.parents.length === 0 || this.parents.every((parent) => parent.done)) && !this.isPorcessing && !this.done;
  }
}
class Worker {
  constructor(step, timeLeft) {
    this.step = step;
    this.eta = timeLeft;
  }
  init(step, timeLeft) {
...
const stepsMapping = (input) => {
  const stepMap = new Map();
  input.forEach((value) => {
    const [, step, dependency] = value.match(/Step (\w) must be finished before step (\w) can begin./);
    if (!stepMap.has(step)) {
...
const part1 = (rawInput) => {
  ...
  while (stepMap.size - 1 !== result.length) {
    const nextStep = [...stepMap.values()].filter((step) => step.isDoable()).sort((a, b) => {
...
const part2 = (rawInput) => {
  ...
  let time = -1;
  const workers = new Array(5).fill(null).map(() => new Worker(null, 0));
  while (steps.some((step) => !step.done)) {
    ...
    freeWorkers.forEach((worker) => {
      const nextStep = steps.filter((step) => step.isTempDoable()).sort((a, b) => {
        return a.value > b.value ? -1 : 1;
      })[0];
      if (nextStep) {
