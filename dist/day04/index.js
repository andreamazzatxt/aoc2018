...
const parseInput = (rawInput) => rawInput.split("\n").map((entry) => {
  ...
  return {
    ...
    date,
    minutes,
    guard: entry.includes("begins") ? entry.slice(26, entry.indexOf("begins") - 1) : "",
    fallAsleep: entry.includes("falls"),
    wakesUp: entry.includes("wakes")
  };
});
const sortByTime = (input) => {
...
const part1 = (rawInput) => {
  ...
  const array = Object.keys(shifts).map((guard) => {
    return {
      id: guard,
      minutesSleeping: shifts[guard].minutesSleeping.reduce((a, b) => a + b, 0),
      maxMinute: shifts[guard].minutesSleeping.indexOf(Math.max(...shifts[guard].minutesSleeping))
    };
  });
  console.table(array);
